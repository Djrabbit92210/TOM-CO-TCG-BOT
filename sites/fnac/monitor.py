...
class FnacMonitor:
    ...
    def _poll_search(self, http: FnacHttpClient, w: FnacWatchConfig) -> WatchSnapshot:
        ...
        if not ok or is_akamai_challenge(html):
            html2, err = self._html_via_http_or_playwright(
                http,
        ...
        pid = resolve_product_id(
            url=best.get("url"),
            html=None,
            explicit=w.product_id,
        ) or (str(best["product_id"]) if best.get("product_id") else None)
        price = best.get("price_eur")
        add_hint = bool(best.get("add_to_cart_hint"))
        in_band = _price_in_band(price, w.price_min_eur, w.price_max_eur)

        # If search row has no price/cta signal, fetch PDP once for a stronger read
        if pid and (price is None or not add_hint):
            purl = best.get("url") or ""
            if purl:
                phtml, perr = self._html_via_http_or_playwright(
                    http,
                    purl,
                    referer=f"{config.BASE_URL}{path}",
                    warm_path=path,
                )
    ...
    def _explain_url(pid: str | None, purch: bool, in_band: bool, price: float | None) -> str:
        ...
        if not in_band:
            return f"price_out_of_band:{price}"
        return "unknown"
    ...
    def try_purchase(self) -> AddToCartResult | None:
        ...
        if not snap.ready_to_buy or not snap.product_id:
            return None
        return self.buyer.add_to_cart(
            snap.product_id,
            referer_path=snap.referer_path,
        )
    ...
    def run_loop(
        ...
        *,
        ...
        """Poll until purchase succeeds or max_iterations reached."""
        n = 0
        while True:
            n += 1
            ...
            if snap.ready_to_buy and snap.product_id:
                r = self.buyer.add_to_cart(snap.product_id, referer_path=snap.referer_path)
                if r.ok:
                    return r
