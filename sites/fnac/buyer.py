...
class FnacBuyer:
    ...
    def probe_cart_endpoints(self, *, referer_path: str = "/") -> dict[str, Any]:
        """After add-to-cart, call inferred GETs to see which JSON routes exist."""
        with FnacHttpClient(cookie_store=self.cookie_store, user_agent=self.user_agent) as http:
            http.warm_session(referer_path=referer_path)
            page = http.get_basket_page()
            cand = http.get_basket_json_candidates()
            return {
                "basket_page": {"status": page.status_code, "snippet": page.text[:800]},
                "json_candidates": {k: {"status": v.status_code, "body": v.body} for k, v in cand.items()},
            }

    def probe_cart_via_playwright(
        self,
        *,
        referer_path: str = "/SearchResult/ResultList.aspx?Search=pokemon&sft=1&sa=0",
        paths: list[str] | None = None,
    ) -> dict[str, Any]:
        """GET /basket* via Playwright (works when httpx hits Akamai)."""
        from sites.fnac.playwright_fnac import get_with_session_playwright
        from sites.fnac.response_summary import summarize_fnac_response

        targets = paths or ["/basket", "/basket/summary", "/basket/lines"]
        out: dict[str, Any] = {}
        for path in targets:
            st, txt = get_with_session_playwright(
                path,
                referer_path=referer_path,
                cookie_store=self.cookie_store,
                user_agent=self.user_agent,
            ...
            out[path] = {
                "http_status": st,
                "summary": summarize_fnac_response(st, txt),
            }
        return out
...
class FnacSite:
    ...
    def buy(self, product: dict[str, Any], account: Any, payment: Any) -> bool:
        ...
        r = self._buyer.add_to_cart(str(pid), **kwargs)
        return r.ok
