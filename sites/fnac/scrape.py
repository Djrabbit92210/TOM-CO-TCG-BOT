...
def parse_search_results(html: str) -> list[Listing]:
    ...
    Uses both legacy /a digits links and modern f-productCard classes.
    """
    soup = BeautifulSoup(html, "html.parser")
    seen: set[str] = set()
    out: list[Listing] = []

    # Strategy 1: Look for explicit product card containers (modern Fnac)
    cards = soup.select(".f-productCard, .Article-item, [data-query-id]")
    for card in cards:
        a = card.select_one("a.f-productCard__link, a[href*='/a']")
        if not a:
            continue
        ...
        # If title is still weak, try the direct h2/p inside the card
        if len(title) < 5:
            title_node = card.select_one(".f-productCard__title, [data-automation='product-title']")
            if title_node:
                title = title_node.get_text(" ", strip=True)

        add_ok, price = _block_signals(card.decode().lower())
    ...
    # Strategy 2: Fallback to scanning all links (legacy/backup)
    if not out:
        for a in soup.find_all("a", href=True):
            ...
            for _ in range(6):
                ...
                node = node.parent
            block = "\n".join(chunks)
            add_ok, price = _block_signals(block.lower())
